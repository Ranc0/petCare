...
def doTheJob(user_input):
    ...
    if len(user_input.strip()) < 5 or len(matched_symptoms) == 0:
        ...
        result = (
            f"Based on what you've described, your dog may be showing signs of **{disease_name}**.\n\n"
            f"Confidence: {confidence:.1f}%\n\n"
            f"Suggestion: {care_tips[disease_name]}\n\n"
            f"Common Symptoms: {', '.join(disease_info[disease_name]['Symptoms'])}\n"
            f"Possible Causes: {', '.join(disease_info[disease_name]['Causes'])}\n\n"
            "If symptoms persist or worsen, contact a veterinary clinic for confirmation.\n\n"
            "Please note: This diagnosis was generated by an AI-based system and may not be accurate. "
            "It is not a substitute for professional veterinary advice. For your pet's safety, consult a licensed veterinarian as soon as possible.\n\n"
            f"{top_pred_text}"
        )
